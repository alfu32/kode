...
- Project state: Kode writes `.kode.json` in the project root to persist recent files and open editor state (cursor/selection/scroll + unsaved buffer text, no grammars). On startup, it restores the project session, including recently opened files and their editor positions.
