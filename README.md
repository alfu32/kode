...
- Code intelligence: project files are pre-indexed into a persistent semantic database of symbols, scopes, occurrences, types, imports, relations, and typed file dependencies. Kotlin currently uses scope-aware symbol identity, progressive assignment/call/cast inference, structured nullable/generic/union types, visibility-aware cross-file and inherited member lookup, and chained `receiver.`/`receiver?.` completion including `this` and `super`; interactive edits re-resolve only the changed file unless its exported semantic surface invalidates dependent files. Other languages continue through the legacy heuristic adapter during migration. Tree-sitter supplies transient syntax, while LSP remains optional enrichment. Declarations render blue + bold and usages blue + italic underline; Ctrl-click navigates definitions/usages and Ctrl+Space opens ranked suggestions.
...
- Working folder: defaults to the shell directory you launch from; pass `kode <path>` (e.g., `kode .` or `kode x/y/z`) to open a different root and read/write `.kode.json` there, or click `[change]` in the Files tab header to pick a new workspace folder. Source indexing scans only the folders listed in the Project panel, applies `.gitignore`, and supports persisted file/folder exclusions from that panel.
