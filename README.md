...
- Code intelligence: project files are pre-indexed into a persistent semantic database of symbols, scopes, occurrences, types, imports, relations, and typed file dependencies. Kotlin currently uses scope-aware symbol identity, progressive type inference, cross-file and inherited member lookup, and `receiver.` completion; interactive edits re-resolve only the changed file unless its exported semantic surface invalidates dependent files. Other languages continue through the legacy heuristic adapter during migration. Tree-sitter supplies transient syntax, while LSP remains optional enrichment. Declarations render blue + bold and usages blue + italic underline; Ctrl-click navigates definitions/usages and Ctrl+Space opens ranked suggestions.
