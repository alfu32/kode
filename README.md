...
- Code intelligence: project files are pre-indexed into a persistent semantic database of symbols, scopes, occurrences, types, imports, and relations. Kotlin currently uses scope-aware symbol identity, progressive type inference, cross-file and inherited member lookup, and `receiver.` completion; other languages continue through the legacy heuristic adapter during migration. Tree-sitter supplies transient syntax, while LSP remains optional enrichment. Declarations render blue + bold and usages blue + italic underline; Ctrl-click navigates definitions/usages and Ctrl+Space opens ranked suggestions.
