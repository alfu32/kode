...
class WorkspacePickerDialog(
    ...
    override fun dispatch(event: UIEvent): Boolean {
        when (event.kind) {
            ...
            "mouse_down" -> {
                ...
                if (entry.typ == "folder") {
                    // allow toggling on prefix click or anywhere in row
    ...
    private fun renderHeader(canvas: CanvasRenderer, x: Int, y: Int, width: Int) {
        val title = "Select workspace folder"
    ...
    private fun renderFooter(canvas: CanvasRenderer, x: Int, y: Int, width: Int) {
        ...
        val label = selectedPath.take(width - "Selected: ".length).padEnd((width - "Selected: ".length).coerceAtLeast(0), ' ')
        canvas.withStyle(style) {
    ...
    private fun directoryEntries(): List<FileTreeEntry> {
        val rootEntry = FileTreeEntry(
            name = Path.of(tree.root).fileName?.toString() ?: tree.root,
            typ = "folder",
            padding = 0,
            fullPath = tree.root,
            isOpen = true
